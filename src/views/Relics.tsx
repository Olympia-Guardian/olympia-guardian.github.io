...
const EXPANSIONS: { num: number; fr: string; en: string }[] = [
  ...
  {
    num: 0,
    fr: "Donjons spéciaux & Ultimates",
    en: "Special dungeons & Ultimates",
  },
];
...
function matsText(
  mats: { qty: number; fr: string; en: string }[],
  lang: string,
): string {
  return mats
    .map((mat) => `${fmt(mat.qty, lang)} ${lang === "fr" ? mat.fr : mat.en}`)
    .join(" · ");
}
...
function RelicPanel({
  ...
  const steps =
    info.stepSizes?.length ?? Math.max(1, Math.round(info.total / info.jobs));
  ...
  return (
    <aside className="item-panel">
      ...
      <img
        ...
        onError={onItemImgError}
      />
      <h3 className="item-panel-name">
        {lang === "fr" ? relic.name : relic.nameEn}
      </h3>
      {lang === "fr" && relic.nameEn !== relic.name && (
        <p className="modal-en">{relic.nameEn}</p>
      )}
      <p className="modal-chips">
        <span className="chip chip-type">{t(catKey)}</span>
        <span className="chip chip-patch">
          {lang === "fr" ? info.name : info.key}
        </span>
        {steps > 1 && info.category !== "garo" && (
          <span className="chip chip-patch">
            {costs?.stepLabels?.[step]
              ...
              : t(info.category === "armor" ? "relicTier" : "relicStep", {
                  n: step + 1,
                })}
          </span>
        )}
      ...
      {(() => {
        const url =
          lang === "en"
            ? (stepCost?.urlEn ?? costs?.urlEn ?? stepCost?.url)
            : stepCost?.url;
        return url ? (
          <p className="modal-desc">
            <a
              className="relic-guide"
              href={url}
              target="_blank"
              rel="noreferrer"
            >
...
const SeriesCard = memo(function SeriesCard({
  ...
  const garoSets =
    info.category === "garo" &&
    relics.some((r) => /^The \w+ of /.test(r.nameEn));
  ...
  const costs = RELIC_COSTS[info.key];
  const costSteps: StepCost[] | null = costs
    ? effectiveSteps(costs, steps)
    : null;
  ...
  const roleOf = (r: Relic) =>
    ROLE_ORDER.indexOf(
      (r.nameEn.match(
        / of (Fending|Maiming|Striking|Scouting|Aiming|Casting|Healing)\b/,
      )?.[1] ?? "") as (typeof ROLE_ORDER)[number],
    );
  const sortArmor = (list: Relic[]): Relic[] => {
    if (!isArmor) {
      ...
      return [...list].sort(
        (a, b) => jobRank(a) - jobRank(b) || a.order - b.order,
      );
    }
    return [...list].sort((a, b) => {
      ...
      if (ra === -1)
        return a.nameEn.localeCompare(b.nameEn) || a.order - b.order;
  ...
  const armorGroups = (
    list: Relic[],
  ): { label: string | null; icon?: string; items: Relic[] }[] => {
    if (garoSets) {
      ...
      for (let j = 0; j < list.length; j += 5) {
        ...
        out.push({
          label: label(items[0]),
          sort: GARO_SORT[enKey(items[0])] ?? 99,
          items,
        });
      }
  ...
  const ownedInStep = useCallback(
    (memberId: number, i: number) => {
      ...
      if (!owned) return 0;
      return stepRelics[i].reduce(
        (sum, r) => sum + (owned.has(r.id) ? 1 : 0),
        0,
      );
    },
  ...
  const chained =
    steps > 1 && !info.stepSizes && !garoSets && !costs?.independentSteps;
  const chainFor = (relic: Relic, step: number): RelicChain | undefined => {
    ...
    for (let s = 0; s < stepRelics.length; s++) {
      if (s === step) continue;
      const match = stepRelics[s].find(
        (r) => (r.order - 1) % info.jobs === pos,
      );
  ...
  const totalMats = useMemo(() => {
    ...
    costSteps.forEach((st, i) => {
      ...
      const started =
        ready.length === 1 &&
        (stepRelics[i]?.length ?? 0) > 0 &&
        ownedInStep(ready[0].id, i) > 0;
  ...
  const compactQty = (n: number) =>
    n >= 10000 ? `${Math.round(n / 1000)}k` : fmt(n, lang);
  ...
  const matIcon = (mat: Material) => {
    ...
    return (
      <span
        key={mat.key ?? mat.en}
        className="relic-mat-chip"
        title={`${fmt(mat.qty, lang)} ${full}`}
      >
        {mat.icon && (
          <img
            src={mat.icon}
  ...
  return (
    <article className="relic-series">
      <header className="relic-series-head">
        ...
        <span className="relic-shape">
          {info.stepSizes
            ...
            : steps > 1
              ? t(isArmor ? "relicShapeNArmor" : "relicShapeN", {
                  steps,
                  jobs: info.jobs,
                })
              : garoSets
                ? t("relicShapeGaro", { sets: Math.round(info.total / 5) })
                : t(isArmor ? "relicShape1Armor" : "relicShape1", {
                    jobs: info.jobs,
                  })}
        </span>
      </header>
          ...
          return (
            <div key={member.id} className="relic-player">
              ...
              <div className="relic-meter">
                <Meter
                  label={member.data.name.split(" ")[0]}
                  count={count}
                  total={info.total}
                />
              </div>
              ...
              {stepRelics.map((list, i) => {
                ...
                return (
                  <div key={i} className="relic-icon-step">
                        ...
                        <span className="relic-step-head">
                          {steps > 1 && (
                            <b>
                              {stepLabel(i)} ·{" "}
                              <span
                                className={c >= list.length ? "relic-done" : ""}
                              >
                                {c}/{list.length}
                              </span>
                            </b>
                          ...
                          {stepCost &&
                            stepCost.materials.length > 0 &&
                            showReq && (
                              <span className="relic-req-items">
                                {reqLeft &&
                                remainingMats &&
                                list.length - c === 0 ? (
                                  <span className="relic-done">
                                    {t("relicDone")}
                                  </span>
                                ) : (
                    ...
                    <div
                      className={
                        isArmor || garoSets ? "relic-icon-groups" : undefined
                      }
                    >
                      {armorGroups(list).map((g, gi) => (
                        <div
                          key={gi}
                          className={
                            isArmor || garoSets ? "relic-icon-group" : undefined
                          }
                        >
                          {g.label && (
                            ...
                            {g.items.map((r) => {
                              ...
                              const content = (
                                <>
                                  ...
                                  />
                                  {has && (
                                    <span className="relic-badge">✓</span>
                                  )}
                                </>
                              );
                              return onRelicClick ? (
      ...
      {ready.length > 1 &&
        steps === 1 &&
        costSteps?.[0] &&
        costSteps[0].materials.length > 0 && (
...
const parAvance = (a: Rang, b: Rang) =>
  b.count / (b.total || 1) - a.count / (a.total || 1);
...
function BarreJoueur({ r, lang }: { r: Rang; lang: string }) {
  ...
  return (
    <li className="prog-row">
      ...
      <span className={`prog-pct ${fini ? "relic-done" : ""}`}>
        {pct(r.count, r.total, lang)}
      </span>
    </li>
  );
}
...
function RelicSummary({
  ...
  const headerTotal = cdb
    ? totalAll + KINDS.reduce((s, k) => s + cdb[k].length, 0)
    : totalAll;
    ...
    () =>
      new Map(
        ready.map((m) => {
          ...
          return [
            m.id,
            db.relics.reduce((sum, r) => sum + (owned.has(r.id) ? 1 : 0), 0),
          ];
        }),
      ),
    [ready, ownedSets, db],
      ...
      <section className="relic-series relic-global">
        <header className="relic-series-head">
          <h4 className="relic-series-name">
            {t(cdb ? "groupProgress" : "relicGlobal")}
          </h4>
...
export function Relics({
    ...
    for (const member of ready) {
      ...
      for (const info of db.series) {
        ...
        const missingPerStep = Array.from({ length: steps }, (_, i) => {
          const inStep = relics.filter(
            (r) => Math.ceil(r.order / info.jobs) === i + 1,
          );
        ...
        const rem = remainingMaterials(
          costs,
          missingPerStep,
          info.jobs,
          origin,
        );
        for (const mat of [...rem.perWeapon, ...rem.once])
          mergeMaterial(acc, mat);
      }
      perPlayer.set(
        member.id,
  ...
  const handleRelic = onToggleRelic
    ? (info: RelicSeriesInfo) =>
        (relic: Relic, step: number, chain?: RelicChain) => {
  ...
  return (
    <div className="view">
      ...
      {pick && onToggleRelic && (
        <RelicPanel
          pick={pick}
          owned={(ownedSets.get(ready[0]?.id ?? 0) ?? new Set()).has(
            pick.relic.id,
          )}
          ...
          {ready.map((m) => {
            ...
            return (
              <div key={m.id} className="relic-player">
                ...
                <div className="relic-meter">
                  <Meter
                    label={m.data.name.split(" ")[0]}
                    count={count}
                    total={totalAll}
                  />
                </div>
                <span className="relic-remaining">
                  {pct(count, totalAll, lang)}
                </span>
              </div>
            );
          })}
              ...
              <div key={member.id} className="relic-grand-player">
                ...
                {currencies.length > 0 && (
                  <div className="relic-grand-line">
                    <span className="relic-grand-cat">
                      {t("relicMatCurrency")}
                    </span>
                    <span className="relic-grand-items">
                      {currencies.map((mat) => grandChip(mat, lang))}
                    </span>
                  </div>
                )}
                {items.length > 0 && (
                  <div className="relic-grand-line">
                    ...
                    <span className="relic-grand-items">
                      {items.map((mat) => grandChip(mat, lang))}
                    </span>
                  </div>
                )}
                {drops.length > 0 && (
                  <div className="relic-grand-line">
                    ...
                    <span className="relic-grand-items">
                      {drops.map((mat) => grandChip(mat, lang))}
                    </span>
                  </div>
                )}
              </div>
        ...
        return (
          <details key={num} className="relic-cat" open={num === 7}>
            <summary className="relic-cat-head">
              {lang === "fr" ? fr : en}{" "}
              <span className="relic-cat-count">({series.length})</span>
            </summary>
