...
function armoireSectionKey(
  it: Item,
): "armoireArmor" | "armoireAcc" | "armoireWeapons" | "armoireTools" {
...
function localItemName(it: Item, lang: string): string {
  return (
    (lang === "fr" ? it.itemName : (it.itemNameEn ?? it.itemName)) ?? it.name
  );
}
...
function depuisQuand(
  iso: string,
  lang: string,
  t: (
    k: "syncNow" | "syncMin" | "syncHours" | "syncDays",
...
function SourceIcon({
  ...
  s: { type: string; text: string; textEn: string };
}) {
...
function ItemPanel({
  ...
  return (
    <aside className="item-panel">
      <button
        className="icon-btn item-panel-close"
        title={t("close")}
        onClick={onClose}
      >
        ×
      ...
      {item.nameEn !== localName(item, lang) && (
        <p className="modal-en">{item.nameEn}</p>
      )}
      <p className="modal-chips">
        {item.patch && (
          <span className="chip chip-patch">
            {t("patch", { n: item.patch })}
          </span>
        )}
        {item.tradeable && (
          ...
          {item.pieces.map((p) => {
            ...
            return (
              <li key={p.id}>
                ...
                >
                  <span
                    className={`checklist-box ${hasPiece ? "is-owned" : ""}`}
                  >
                    {hasPiece ? "\u2713" : ""}
                  </span>
      ...
      {item.sources.length > 0 && (
        <ul className="modal-sources">
          {item.sources.map((s, i) => (
            <li key={i}>
              <TypeChip type={s.type} /> <SourceIcon s={s} />{" "}
              {localSource(s, lang)}
            </li>
          ))}
        </ul>
          ...
          >
            ...
            <span className="spell-main">
              <span className="spell-top">
                ...
                <span className="spell-no">
                  {t("spellNo", { n: it.order })}
                </span>
              </span>
              <span className="spell-mid">
                ...
                <span className="spell-stars" title={`${it.rank ?? 0}/5`}>
                  {Array.from({ length: 5 }, (_, i) => (
                    <span
                      key={i}
                      className={i < (it.rank ?? 0) ? "is-filled" : ""}
                    >
                      ★
                    </span>
                  ))}
              ...
              <span className="spell-src">
                {it.sources[0] && (
                  <>
                    {it.sources[0].type !== "Other" && (
                      <TypeChip type={it.sources[0].type} />
                    )}
                    <SourceIcon s={it.sources[0]} />
                    <span className="spell-src-text">
                      {localSource(it.sources[0], lang)}
                    </span>
                  </>
                )}
              </span>
...
function CardAlbum({
  ...
  const shows = (it: Item) =>
    visible.has(it.id) &&
    (!srcType || it.sources.some((s) => s.type === srcType));
  ...
  return (
    <div className="cat-layout album-wrap">
      <CatRail
        ...
        all={{
          owned: allItems.reduce(
            (sum, it) => sum + (ids.has(it.id) ? 1 : 0),
            0,
          ),
...
/** Une page de l'album. Ses cartes ne sont rendues qu'a l'approche de l'ecran :
 *  dix-sept pages, 475 images, dont 385 partaient autrefois sans que personne
 *  ne les voie. */
function PageAlbum({
  label,
  cells,
  ...
  ids: Set<number>;
  lang: Lang;
  numero: (it: Item) => number | string;
  shows: (it: Item) => boolean;
  onItemClick: (it: Item) => void;
}) {
  const [ref, vu] = useVisible<HTMLElement>();
  return (
    <section ref={ref} className={`album-page ${isEx ? "album-page-ex" : ""}`}>
      <header className="album-page-head">
        <b>{label}</b>
        <span className={`mypage-count ${owned === total ? "relic-done" : ""}`}>
          {owned}/{total}
        </span>
      </header>
      <div className="album-grid">
        {cells.map((it, slot) => {
          if (!it || !shows(it))
            return <span key={slot} className="album-slot" />;
          const has = ids.has(it.id);
          return (
            <button
              key={it.id}
              className={`album-card ${has ? "is-owned" : "is-missing"}`}
              title={`${localName(it, lang)} · ${it.num ?? `n°${numero(it)}`}${has ? " ✓" : ""}`}
              onClick={() => onItemClick(it)}
            >
              {/* La carte n'a pas d'image tant que la page n'approche
                          pas : la case garde sa taille, rien ne saute. */}
              {vu && (
                <img
                  src={it.image}
                  alt=""
...
function IconGrid({
  ...
  const shown = cat
    ? items.filter(
        (it) => ((lang === "fr" ? it.group : it.groupEn) ?? "") === cat,
      )
    : items;
                ...
                return (
                  <section key={label}>
                    <header className="album-page-head">
                      <b>{label}</b>
                      <span
                        className={`mypage-count ${owned === list.length ? "relic-done" : ""}`}
                      >
                        {owned}/{list.length}
                      </span>
                    </header>
...
function GroupedChecklist({
  ...
  const allGroups = useMemo(() => {
    ...
    for (const it of [...items].sort(
      (a, b) => numOf(a) - numOf(b) || a.order - b.order,
    )) {
      const g = (lang === "fr" ? it.group : it.groupEn) ?? "";
      const arr = map.get(g);
      if (arr) arr.push(it);
                ...
                {list.map((it) => {
                  ...
                  return (
                    <li key={it.id}>
                      ...
                      >
                        <span
                          className={`checklist-box ${has ? "is-owned" : ""}`}
                        >
                          {has ? "✓" : ""}
                        </span>
                        {variant === "icon" && (
                        ...
                        {variant === "orchestrion" && it.num !== undefined && (
                          <span className="checklist-num">
                            {/^\d+$/.test(String(it.num))
                              ? String(it.num).padStart(3, "0")
                              : "—"}
                          </span>
                        )}
                        <span
                          ...
                          title={
                            it.itemName ? localItemName(it, lang) : undefined
                          }
                        >
                          {localName(it, lang)}
                        </span>
                        ...
                        {it.command && (
                          <span className="chip chip-cmd">{it.command}</span>
                        )}
                        {it.patch && (
                          <span className="chip chip-patch">{it.patch}</span>
                        )}
                        {(() => {
                          if (!pieceOwned || !it.pieces?.length || has)
                            return null;
                          const owned = it.pieces.filter((pc) =>
                            pieceOwned.has(pc.id),
                          ).length;
                          return (
                            <span
...
function FashionEditor({
  ...
  const { lang, t } = useI18n();
  const [idsByKind, setIdsByKind] = useState<
    Partial<Record<Kind, Set<number>>>
  >(() =>
  ...
  const saveTimers = useRef<
    Partial<Record<Kind, ReturnType<typeof setTimeout>>>
  >({});
    ...
    () =>
      new Set(
        FASHION_KINDS.flatMap((k, ki) =>
          [...(idsByKind[k] ?? [])].map((id) => ki * FASHION_NS + id),
        ),
      ),
    [idsByKind],
  );
...
function CollectionEditor({
  ...
  const [pieceIds, setPieceIds] = useState<Set<number>>(
    () => new Set(pieceOwned ?? []),
  );
  ...
  const shownIds = useMemo(() => {
    ...
    for (const it of db.outfits) {
      if (
        ...
        it.pieces!.every((pc) => pieceIds.has(pc.id))
      )
        s.add(it.id);
    }
    return s;
  ...
  return (
    <div className="mypage-editor">
      <div className="controls editor-controls">
        ...
        {achPts && (
          <span
            className="chip chip-type ach-points"
            title={t("achPoints", { n: achPts.own })}
          >
            🏆{" "}
            {t("achPointsChip", {
              a: achPts.own.toLocaleString(lang),
              b: achPts.tot.toLocaleString(lang),
            })}
          </span>
        )}
        {!readOnly && (
      ...
      <div className="editor-layout">
        <div className="editor-body">
          {kind === "cards" ? (
            ...
            <GroupedChecklist
              ...
              onPieceToggle={
                kind === "outfits" && !readOnly ? togglePieceOf : undefined
              }
            />
          ) : (
            <IconGrid
        ...
        {selected && (
          <ItemPanel
            ...
            pieceOwned={kind === "outfits" ? pieceIds : undefined}
            onTogglePiece={
              kind === "outfits" && !readOnly
                ? (pid: number) => togglePieceOf(selected, pid)
                : undefined
            }
          />
        )}
...
export function MyPage({
  ...
  const [searchResults, setSearchResults] = useState<CharSearchResult[] | null>(
    null,
  );
  ...
  const verified =
    verifiedList.find((b) => b.charId === activeId) ?? verifiedList[0];
  ...
  async function doSearch() {
    ...
    setNotice(null);
    try {
      const r = await apiSearchCharacter(
        auth.token,
        searchName.trim(),
        searchServer.trim() || undefined,
      );
  ...
  async function forceSync() {
    ...
    try {
      ...
      setChar(fresh);
      showToast(
        t(fresh.nextForceAt > before ? "syncForceDone" : "syncForceAlready"),
      );
    } catch {
  ...
  function setRelics(ids: number[], add: boolean) {
    ...
    relicSaveTimer.current = setTimeout(
      () => save("relics", relicIdsRef.current),
      1200,
    );
  }
    ...
    <div className="view mypage">
      ...
      {(!verified || adding) && (
        <section className="relic-series mypage-bind">
          <h3 className="relic-series-name">
            {verified ? t("bindAdd") : t("bindTitle")}
          </h3>
          ...
          <ol className="onboard-steps">
            ...
            <li className={pending ? "is-done" : "is-active"}>
              <span className="onboard-num">{pending ? "✓" : "2"}</span>{" "}
              {t("onboardStep2")}
            </li>
          ...
          {pending && (
            <>
              <ol className="onboard-verify">
                <li>
                  {t("verifyStepCopy")}{" "}
                  <code className="bind-code">{pending.code}</code>{" "}
                  <button
                ...
                <div className="player-head mypage-char-head">
                  ...
                  <span className="mypage-char-actions">
                    ...
                    <span
                      ...
                      title={new Date(char.lastParsed).toLocaleString(
                        lang === "fr" ? "fr-FR" : "en-US",
                      )}
                    >
                      {t("syncLast", {
                        quand: depuisQuand(char.lastParsed, lang, t),
                      })}
                    </span>
                    <button
                      className="btn btn-ghost btn-mini"
                        ...
                        Date.now() < char.nextForceAt
                          ? t("syncForceCooldown", {
                              h: Math.max(
                                1,
                                Math.ceil(
                                  (char.nextForceAt - Date.now()) / 60_000,
                                ),
                              ),
                            })
                          : t("syncForceTitle")
                    ...
                    >
                      <TabIcon k="sync" />{" "}
                      {syncing ? t("syncRunning") : t("syncForce")}
                    </button>
                ...
                <div className="meter-grid mypage-meters">
                  {KINDS.filter(
                    ...
                    const count = merged
                      ? char.fashions.count +
                        char.facewear.count +
                        char.hairstyles.count
                      : char[k].count;
                    const total = merged
                      ? char.fashions.total +
                        char.facewear.total +
                        char.hairstyles.total
                      : char[k].total;
                    const countAll = merged
                      ? char.fashions.countAll +
                        char.facewear.countAll +
                        char.hairstyles.countAll
                      : char[k].countAll;
                    const totalAll = merged
                      ? char.fashions.totalAll +
                        char.facewear.totalAll +
                        char.hairstyles.totalAll
                      : char[k].totalAll;
                    return (
                      <Meter
                        key={k}
